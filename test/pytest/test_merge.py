...
@pytest.mark.parametrize('backend', ['Vivado', 'Vitis', 'Quartus'])
@pytest.mark.parametrize('swap_inputs', [True, False])
def test_merge_bcast(merge_layer, io_type, backend, swap_inputs):
    input_shape1 = (1, 1, 3)
    input_shape2 = (2, 5, 3)
    ...
    if swap_inputs:
        out = merge_layer()([in2, in1])
        model = tf.keras.models.Model(inputs=[in2, in1], outputs=out)
    else:
        out = merge_layer()([in1, in2])
        model = tf.keras.models.Model(inputs=[in1, in2], outputs=out)
    ...
    output_dir = str(
        test_root_path
        / f'hls4mlprj_merge_bcast_{"swap_inputs_" if swap_inputs else ""}{merge_layer.__name__.lower()}_{backend}_{io_type}'
    )
    hls_model = hls4ml.converters.convert_from_keras_model(
        model, hls_config=config, output_dir=output_dir, io_type=io_type, backend=backend
    )
    ...
    X_input1 = np.random.rand(4, *input_shape1)
    X_input2 = np.random.rand(4, *input_shape2)

    if swap_inputs:
        keras_prediction = model.predict([X_input2, X_input1])
        hls_prediction = hls_model.predict([X_input2, X_input1]).reshape(keras_prediction.shape)
    else:
        keras_prediction = model.predict([X_input1, X_input2])
        hls_prediction = hls_model.predict([X_input1, X_input2]).reshape(keras_prediction.shape)
