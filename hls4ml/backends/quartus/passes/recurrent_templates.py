...
class LSTMConfigTemplate(LayerConfigTemplate):
    ...
    def format(self, node):
        ...
        lstm_params['sliding_window'] = 'false' #str(node.get_attr('Sliding_window')).lower()
...
class SimpleRNNConfigTemplate(LayerConfigTemplate):
    ...
    def format(self, node):
        ...
        simple_rnn_params['n_timestamp'] = node.get_attr('n_timestamp', 5)
        simple_rnn_params['sliding_window'] = 'false' #str(node.get_attr('Sliding_window')).lower()
...
class SimpleRNNFunctionTemplate(FunctionCallTemplate):
    ...
    def format(self, node):
        ...
        params['weights'] = 'kernel_{0}, recurrent_kernel_{0}, bias_{0}'.format(str(node.index))
